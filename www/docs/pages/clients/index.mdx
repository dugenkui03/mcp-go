...
```go
// Create STDIO client
client, err := client.NewStdioMCPClient("server-command", nil, "arg1", "arg2")
```
...
client, err := client.NewStreamableHttpClient("http://localhost:8080/mcp")
```
...
```go
// Create SSE client
client, err := client.NewSSEMCPClient("http://localhost:8080/mcp/sse")
```
...
func NewMCPClientManager(clientType, address string) (*MCPClientManager, error) {
    ...
    switch clientType {
    case "stdio":
        c, err = client.NewStdioMCPClient("server-command", nil)
    case "http":
        c, err = client.NewStreamableHttpClient(address)
        if err != nil {
